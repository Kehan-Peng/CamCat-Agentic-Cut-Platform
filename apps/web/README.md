...
当前工作台已连接真实后端链路：视频上传与索引任务、文本/图片多模态检索、Evidence/Route/Trace、Editing Session、多轮 Agent Domain Command、版本冲突、补偿式回滚、FFmpeg 渲染、成片播放与下载。静态内容仅用于尚无数据时的视觉空状态，不会作为 API fallback。
